...
## Running it against a real GPU

There are three ways to run the same system. The control plane — queue,
revisions, retries, artifacts, evaluation, review, monitoring — is identical in
all three; only the executor changes.

| `COMFY_MODE` | Executor | GPU | Use it for |
|---|---|---|---|
| `fake` (default) | bundled simulator | none | The whole loop with no GPU and no spend — this is `pnpm dev` above |
...
too, pointed at `127.0.0.1`.

`fake` is not a lesser mode. It implements the HTTP/WebSocket protocol the
worker needs and emits deterministic media from a tracked fixture, and it is the
only way to test the recovery paths: a real GPU cannot be asked to drop a
WebSocket, emit a duplicate event, or return an uncertain submission on demand.

### What a real executor needs

The same on your machine or a rented one:

- **A GPU with at least 32 GB of VRAM.** The verified run peaked at 31.9 GB of
  an RTX 5090's 32.6 GB.
- **About 44 GB of model files** and roughly 120 GB of disk with the checkout,
  virtualenv, and outputs.
- **A CUDA 13-capable driver.** The verified run used `torch 2.14.0+cu130`,
  compute capability 12.0.

Expect the first bring-up to be dominated by downloading the weights, not by
...
1. **Clone ComfyUI at the pinned ref** from
   ...
   CUDA 13 index.
2. **Place the five model files** under the ComfyUI checkout:
...
3. **Install the VideoOps bridge** by copying
...
5. **Set the executor in `.env`** and start VideoOps with `pnpm dev`, which
   skips the fake service in this mode:
...
6. **Confirm the executor before submitting.** `GET /v1/executor` must report
   ready, and the opt-in contract check should pass once per host:

   ```sh
   COMFY_LIVE_TEST=1 COMFY_MODE=remote pnpm test:comfy-live
   ```

Then use it exactly as in fake mode: open ComfyUI, load the H3 graph, and choose
**Managed Run**. Every run is queued, evaluated, reviewed, and recorded the same
way — that is the point of the control plane being executor-agnostic.

### On a rented GPU server

Same six steps, with the whole VideoOps stack on the rented host and ComfyUI
bound to loopback, so no generated byte crosses a network and the private-route
invariant holds by construction. Because the host is disposable, treat its
PostgreSQL data and `ARTIFACT_ROOT` as evidence to export before teardown rather
than as storage.

[`infra/gpu-executor/README.md`](infra/gpu-executor/README.md) is the full host
runbook, and it covers what this section does not: the other two topologies, the
browser gateway that denies `POST /prompt` and queue mutation, keeping models on
a volume that outlives the host, the cost breaker, and teardown.
