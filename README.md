...
You do not need a local checkout. If your provider runs a command at boot, hand it
this and the host builds itself while you are still connecting:

```sh
curl -fsSL https://raw.githubusercontent.com/goAustin/AutoCamera/main/scripts/setup/gpu-rented.sh | bash
```

The same line works over SSH on a host that is already up.
...
API, and stops — having submitted and destroyed nothing. It clones this repository
itself, so the command above is the only thing you need to get onto the host.
